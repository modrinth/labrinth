...
use crate::{
    database::{
        models::{version_item, DatabaseError},
        redis::RedisPool,
    },
    models::ids::VersionId,
    util::actix::{generate_multipart, MultipartSegment, MultipartSegmentData},
};
...
pub async fn set_side_types_from_versions<'a, E>(
    ...
    E: sqlx::Executor<'a, Database = sqlx::Postgres>,
{
    ...
    if let Some(version_id) = version_id {
        ...
        if let Some(versions_item) = versions_item {
            json["client_side"] = versions_item
                .version_fields
                .iter()
                .find(|f| f.field_name == "client_side")
                .map(|f| f.value.serialize_internal())
                .unwrap_or(json!("required"));
            json["server_side"] = versions_item
                .version_fields
                .iter()
                .find(|f| f.field_name == "server_side")
                .map(|f| f.value.serialize_internal())
                .unwrap_or(json!("server_side"));
        }
    }
    Ok(())
}
...
pub async fn extract_ok_json(response: HttpResponse) -> Result<serde_json::Value, HttpResponse> {
    if response.status() == actix_web::http::StatusCode::OK {
        ...
        let json_value: Value =
            serde_json::from_slice(&bytes).map_err(|_| failure_http_response())?;
        Ok(json_value)
    } else {
        Err(response)
    }
