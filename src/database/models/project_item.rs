        ...
        if !remaining_strings.is_empty() {
            ...
            let db_projects: Vec<QueryProject> = sqlx::query!(
                ...
                (
                    ...
                    FROM temp_versions
                    INNER JOIN loaders_versions lv ON temp_versions.id = lv.version_id
                    INNER JOIN loaders l ON lv.loader_id = l.id
                ),
                temp_mods_gallery_json AS
                (
                    SELECT DISTINCT mod_id,
                    JSONB_AGG(
                        DISTINCT jsonb_build_object(
                ...
                LEFT JOIN temp_mods_gallery_json mg ON mg.mod_id = m.id
                LEFT JOIN temp_links_json ml ON ml.mod_id = m.id
                LEFT JOIN mods_categories mc ON mc.joining_mod_id = m.id
                LEFT JOIN categories c ON mc.joining_category_id = c.id
                LEFT JOIN temp_versions_json v ON v.mod_id = m.id
                LEFT JOIN temp_loaders l on l.mod_id = m.id
                ...
                LEFT OUTER JOIN temp_version_fields_json vf ON m.id = vf.mod_id
                LEFT OUTER JOIN temp_loader_fields_json lf ON m.id = lf.mod_id
                LEFT OUTER JOIN temp_loader_field_enum_values_json lfev ON m.id = lfev.mod_id
                WHERE m.id = ANY($1) OR m.slug = ANY($2)
