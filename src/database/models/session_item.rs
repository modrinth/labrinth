...
impl Session {
    pub async fn get<'a, E, T: Display + Hash + Eq + PartialEq + Clone + Debug + Send>(
        id: T,
        exec: E,
        redis: &RedisPool,
    ...
    pub async fn get_many<'a, E, T: Display + Hash + Eq + PartialEq + Clone + Debug + Send>(
        session_strings: &[T],
        exec: E,
        redis: &RedisPool,
    ) -> Result<Vec<Session>, DatabaseError>
        ...
        E: sqlx::Executor<'a, Database = sqlx::Postgres>,
    {
        ...
        .fetch(exec)
        .try_filter_map(|x| async move { Ok(Some(SessionId(x.id))) })
