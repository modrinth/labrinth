use crate::common::{database::USER_USER_ID, environment::TestEnvironment, dummy_data::DummyImage, request_data::get_icon_data};
...
async fn create_organization() {
    ...
    for title in [
        ...
        let resp = api.create_organization(title, "theta_description", USER_USER_PAT)
            .await;
    ...
    for description in ["a", &"a".repeat(300)] {
        let resp = api.create_organization("theta", description, USER_USER_PAT)
        .await;
        assert_eq!(resp.status(), 400);
    }

    // Create 'theta' organization
    let resp = api.create_organization("theta", "not url safe%&^!#$##!@#$%^&", USER_USER_PAT).await;
    assert_eq!(resp.status(), 200);

    // Get organization using slug
    let theta = api.get_organization_deserialized("theta", USER_USER_PAT).await;
    assert_eq!(theta.title, "theta");
    assert_eq!(theta.description, "not url safe%&^!#$##!@#$%^&");
    assert_eq!(resp.status(), 200);

    // Get created team
    let members = api.get_organization_members_deserialized("theta", USER_USER_PAT).await;
...
async fn patch_organization() {
    ...
    // Create 'theta' organization
    let resp = api.create_organization("theta", "theta_description", USER_USER_PAT).await;
    assert_eq!(resp.status(), 200);
    ...
    for title in [
        ...
        let resp = api.edit_organization(zeta_organization_id, json!({
            "title": title,
    ...
    let new_title = api.get_organization_deserialized("new_title", USER_USER_PAT).await;
...
async fn add_remove_icon() {
    ...
    let resp = api.edit_organization_icon(zeta_organization_id, Some(
        get_icon_data(DummyImage::SmallIcon)
    ), USER_USER_PAT).await;
    assert_eq!(resp.status(), 204);

    // Get project
    let zeta_org = api.get_organization_deserialized(zeta_organization_id, USER_USER_PAT).await;
    assert!(zeta_org.icon_url.is_some());

    // Icon delete
    // Uses alpha organization to delete added icon
    let resp = api.edit_organization_icon(zeta_organization_id, None, USER_USER_PAT).await;
    assert_eq!(resp.status(), 204);
...
async fn delete_org() {
    ...
    let resp = api.delete_organization(zeta_organization_id, USER_USER_PAT).await;
    assert_eq!(resp.status(), 204);

    // Get organization, which should no longer exist
    let resp = api.get_organization(zeta_organization_id, USER_USER_PAT).await;
