...
use serde_json::json;
use crate::common::{environment::TestEnvironment, database::USER_USER_ID};
...
async fn create_organization() {
    ...
    // - too long slug
    // - not url safe slug
    for title in [zeta_organization_slug, "a", &"a".repeat(100), "not url safe%&^!#$##!@#$%^&*()"] {
    ...
    .uri(&format!("/v2/organization/theta"))
    .append_header(("Authorization", USER_USER_PAT))
    .to_request();
    ...
    // Get created team
    let req = test::TestRequest::get()
    .uri(&format!("/v2/organization/theta/members"))
    .append_header(("Authorization", USER_USER_PAT))
    .to_request();
    let resp = test_env.call(req).await;
    ...
    assert_eq!(value[0]["permissions"], OrganizationPermissions::all().bits());
...
async fn patch_organization() {
    ...
    // - too long slug
    // - not url safe slug
    for title in ["theta", "a", &"a".repeat(100), "not url safe%&^!#$##!@#$%^&*()"] {
    ...
    let req = test::TestRequest::get()
    .uri(&format!("/v2/organization/new_title"))
    .append_header(("Authorization", USER_USER_PAT))
    .to_request();
...
async fn add_remove_icon() {
    ...
    // Get project
    let req = test::TestRequest::get()
    .uri(&format!("/v2/organization/{zeta_organization_id}", zeta_organization_id=zeta_organization_id))
    .append_header(("Authorization", USER_USER_PAT))
    .to_request();
    ...
    test::TestRequest::patch()
            .uri(&format!("/v2/organization/{zeta_organization_id}/icon?ext=png"))
            .append_header(("Authorization", USER_USER_PAT))
...
async fn add_remove_organization_projects() {
    ...
    for alpha in [alpha_project_id, alpha_project_slug] {
        ...
        let req = test::TestRequest::delete()
        .uri(&format!("/v2/organization/{zeta_organization_id}/projects/{alpha_project_id}"))
        .append_header(("Authorization", USER_USER_PAT))
        .to_request();
        ...
        let value : Result<serde_json::Value,_> = test::try_read_body_json(resp).await;
        assert!(value.is_err());
    }
