...
async fn project_permissions_consistency_test() {
    ...
    .full_project_permissions_test(success_permissions, req_gen).await.unwrap();

    // We do a test with more specific permissions, to ensure that *exactly* the permissions at each step are as expected
    let success_permissions = ProjectPermissions::EDIT_DETAILS | ProjectPermissions::REMOVE_MEMBER | ProjectPermissions::DELETE_VERSION | ProjectPermissions::VIEW_PAYOUTS;
    let req_gen = |ctx: &PermissionsTestContext| test::TestRequest::patch()
    .uri(&format!("/v2/project/{}", ctx.project_id.unwrap()))
    .set_json(json!({
        "title": "Example title - changed.",
...
async fn organization_permissions_consistency_test() {
    ...
    PermissionsTest::new(&test_env)
    .full_organization_permissions_tests(success_permissions, req_gen).await.unwrap();
