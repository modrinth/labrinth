...
async fn creating_loader_fields() {
    ...
    let alpha_project_id = &test_env.dummy.as_ref().unwrap().project_alpha.project_id.clone();
    let alpha_project_id = serde_json::from_str(&format!("\"{}\"", alpha_project_id)).unwrap();
    let alpha_version_id = &test_env.dummy.as_ref().unwrap().project_alpha.version_id.clone();
    ...
    // - Patch
    let resp = api.edit_version(alpha_version_id, json!({
        "invalid": "invalid"
    }), USER_USER_PAT).await;
    assert_eq!(resp.status(), 400);
    ...
    for gvs in [vec![], vec!["1.20.1"].repeat(30)] {
        ...
        let resp: actix_web::dev::ServiceResponse = api.add_public_version(version_data, USER_USER_PAT).await;
        assert_eq!(resp.status(), 400);    
    ...
    for bad_type_game_versions in [json!(1), json!([1]), json!("1.20.1"), json!(["client_side"])] {
        // TODO: - Create project 
        // - Create version
        let version_data = get_public_version_creation_data(
            alpha_project_id,
    ...
    let v = api.add_public_version_deserialized(version_data, USER_USER_PAT).await;
    ...
    let v = api.get_version_deserialized(alpha_version_id, USER_USER_PAT).await;
    assert_eq!(v.fields.get("test_fabric_optional").unwrap(), &json!(555));
    ...
    let v = api.add_public_version_deserialized(version_data, USER_USER_PAT).await;
    assert_eq!(v.fields.get("game_versions").unwrap(), &json!(["1.20.1", "1.20.2"]));
...
async fn get_loader_fields() {
    ...
    let game_versions = api.get_loader_field_variants_deserialized("game_versions").await;
    let side_types = api.get_loader_field_variants_deserialized("client_side").await;
    ...
    let side_type_names = side_types.into_iter().map(|x| x.value).collect::<HashSet<_>>();
    assert_eq!(
        side_type_names,
